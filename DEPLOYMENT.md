...
The Vercel production deployment is configured with the canonical contract address and was browser-verified as reachable, reporting `LIVE`, showing project 1, approved cases 1/2, case-2 memory ID `[1]`, and release 1. Browser verification is recorded in `proof/browser-verification.json`; no wallet-originated write is claimed because the verification browser had no injected provider.
