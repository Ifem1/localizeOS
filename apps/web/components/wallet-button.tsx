...
export default function WalletButton() {
  ...
  useEffect(() => { const p = window.ethereum; if (!p) return; let active = true; void Promise.all([p.request({ method: "eth_accounts" }), p.request({ method: "eth_chainId" })]).then(([accounts, chainId]) => { if (!active) return; setAccount((accounts as string[])[0] ?? null); setChain(parseChainId(String(chainId))); }).catch(() => { if (active) setError("Wallet state unavailable."); }); const accounts = (...a: unknown[]) => setAccount((a[0] as string[] | undefined)?.[0] ?? null); const chains = (...a: unknown[]) => setChain(parseChainId(String(a[0]))); const disconnect = () => { setAccount(null); setChain(null); }; p.on?.("accountsChanged", accounts); p.on?.("chainChanged", chains); p.on?.("disconnect", disconnect); return () => { active = false; p.removeListener?.("accountsChanged", accounts); p.removeListener?.("chainChanged", chains); p.removeListener?.("disconnect", disconnect); }; }, []);
